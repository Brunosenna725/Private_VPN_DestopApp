...
import api from "./api"
import { sendLoginVisible, setIsLoggedIn, setToken } from "./store/electron"
...
const Sidebar = () => {
  ...
  const onSignInClick = async () => {
    ...
    if (!res) {
      return setError("PrivateVPN is not accessible at this time, try again later.");
    }
    if (!res?.data?.success) {
  ...
  const onForgotPasswordClick = (event: MouseEvent) => {
    event.preventDefault();
    const shell = window.require('electron').shell as typeof import('electron')["shell"];
    shell.openExternal("https://surfskip.com/reset-password");
  }
  ...
  return (
    <div style={{"box-shadow": "0 0 24px 8px #00000040"}} class="bg-#F0F5F5 h-full w-432px flex flex-col items-center overflow-auto relative color-#00091F">
      <div class="flex flex-col m-auto items-center">
        ...
        <div class="text-26px text-center font-bold m-t-4">Login To PrivateVPN!</div>
        ...
        <div class="flex flex-col gap-4 m-t-8">
          <Input placeholder="E-Mail" type="email" onInput={setUsernameOrEmail} />
          <Input placeholder="Password" type="password" onInput={setPassword}/>
        </div>
        <Show when={error()}><div class="font-10px color-red self-start">{error()}</div></Show>
        <A href="https://surfskip.com/reset-password" onClick={onForgotPasswordClick} class="text-14px text-left w-full m-t-2">Forgot Your Password?</A>

        <div class="flex flex-col gap-6px m-t-10px m-b-4">
          <button class="font-medium cursor-pointer bg-#185BFF color-white rd-8px h-56px w-300px shrink-0 flex items-center justify-center" onClick={onSignInClick}>Sign In</button>
        </div>
        ...
        <span class=" m-t-12px text-14px ">
          ...
          <A href="/register" class="color-#185BFF">Sign up</A>
        </span>
      </div>
    </div>
  )
